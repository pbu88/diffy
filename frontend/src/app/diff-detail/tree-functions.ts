...
export class FileTree {
  ...
  printTree(tree, level) {
    ...
    result += space + '<li>\n';
    result += space +
        '<a href="javascript:void(0);" class="directory directory-visible"><i class="fa fa-folder-open-o"></i><span class="dir-name">' +
        tree.path + '</span></a>\n';
    var that = this;
    ...
    if (tree.files.length > 0) {
      ...
      tree.files.forEach(function(file) {
        ...
        result += '<li>' +
            ...
            '" class="file" data-filename="' + filename + '" data-wrapperid="' + id + '">\n' +
            '        <i class="fa fa-file-code-o"></i><span class="file-name">' + file.path +
            '</span>\n' +
            '    </a>\n' +
            '</li>';
      });
